...
function printHelp(): void {
  console.log(`aaigotchi send-native\n\nEnv:\n  TOKEN_ID=1 TO=0x... AMOUNT_WEI=1000000000000000 RECEIPT_NOTE=...\nUsage:\n  pnpm send:demo\n  pnpm send:demo:broadcast\n`);
}
...
async function main(): Promise<void> {
  ...
  const amountWei = BigInt(envOrThrow("AMOUNT_WEI"));
  ...
  const data = agency.interface.encodeFunctionData("sendNative", [tokenId, target, amountWei, receiptHash]);
  const submission = await submitBankrTransaction({
    to: deployment.agency,
    chainId: Number(chain.chainId),
    data,
    value: 0n,
    description: `aaigotchi send-native token ${tokenId.toString()} on ${parsed.network}`
  });
  const receipt = await waitForReceipt(provider, submission.transactionHash);
  if (Number(receipt.status) !== 1) {
    throw new Error(`send-native reverted: ${submission.transactionHash}`);
  }
